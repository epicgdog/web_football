  ...
  return (
    <>
      ...
      <div>
        <div>
          {
            Object.keys(teams).length > 0 ? Object.keys(teams).map((val) => (<button className="bob" onClick={() => setSelectedTeam(val)}> {val} </button>)) : "loading"
          }
        </div>
        <div>
          <br></br>
          ...
          {
            plrsLoading ? "loading" : players.map((val) => (<p className='players'>{val.name} | {val.mValue}</p>))
          }
        </div>
      </div>
    </>
